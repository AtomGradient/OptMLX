...
def make_prompt_cache(
    ...
    Args:
        ...
        max_kv_size (Optional[int]): If provided and the model does not have a
            ...
            size of ``max_kv_size``
        max_context_length (Optional[int]): If provided, pre-allocate KV cache
            for the given context length to avoid memory growth during inference.
    """
    ...
    if max_context_length is not None:
        # Extract model dimensions from model.args
        ...
        # Infer dtype from model weights
        dtype = mx.float16
        ...
        if n_kv_heads and head_dim:
            ...
            warnings.warn(
                "Could not extract model dimensions for KV Cache pre-allocation, "
...
class KVCache(_BaseCache):
    ...
    def __init__(
        ...
        if max_context_length > 0 and n_kv_heads > 0 and head_dim > 0:
            # Align to step boundary
            max_context_length = (
                (max_context_length + self.step - 1) // self.step * self.step
            )
            B = 1
            self.keys = mx.zeros(
                (B, n_kv_heads, max_context_length, head_dim), dtype=dtype
            )
