...
def setup_arg_parser():
    ...
    parser.add_argument(
        "--use-mmap",
        action="store_true",
        help="Use memory-mapped I/O for zero-copy model weight loading.",
    )
    parser.add_argument(
        "--pre-allocate-kv",
        type=int,
        ...
        help="Pre-allocate KV cache for the given context length (in tokens) "
        "to keep memory usage flat during inference.",
    )
    parser.add_argument(
        "--draft-model",
        type=str,
...
def generate_step(
    ...
    *,
    ...
    Args:
        ...
        max_context_length (int, optional): Pre-allocate KV cache for this context
          length to keep memory usage flat during inference. Default: ``None``.
...
def main():
    ...
    response = generate(
        ...
        max_context_length=args.pre_allocate_kv,
    )
    if not args.verbose:
        print(response)
