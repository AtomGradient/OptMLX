...
class Reader {
 public:
  ...
  virtual void* data_at(size_t offset) const { return nullptr; }
...
class MmapReader : public Reader {
 public:
  explicit MmapReader(std::string file_path);
  ~MmapReader() override;
  ...
  void* data_at(size_t offset) const override;
  size_t file_size() const { return file_size_; }

  allocator::Buffer get_metal_buffer();
 ...
 private:
  ...
  size_t pos_{0};
  std::string label_;
  allocator::Buffer metal_buffer_{nullptr};
};
#endif // _MSC_VER
