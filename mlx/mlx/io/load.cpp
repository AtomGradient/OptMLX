...
MmapReader::MmapReader(std::string file_path) : label_(std::move(file_path)) {
  int fd = open(label_.c_str(), O_RDONLY);
  if (fd < 0)
    return;
  ...
  mmap_ptr_ = mmap(nullptr, file_size_, PROT_READ, MAP_PRIVATE, fd, 0);
  close(fd); // mmap retains the mapping even after fd is closed
...
MmapReader::~MmapReader() {
  if (metal_buffer_.ptr()) {
    allocator::release(metal_buffer_);
  }
  if (mmap_ptr_) {
...
bool MmapReader::good() const {
  return mmap_ptr_ != nullptr;
}

size_t MmapReader::tell() {
  return pos_;
}

void MmapReader::seek(int64_t off, std::ios_base::seekdir way) {
...
void MmapReader::read(char* data, size_t n) {
  std::memcpy(data, static_cast<char*>(mmap_ptr_) + pos_, n);
  pos_ += n;
}

void MmapReader::read(char* data, size_t n, size_t offset) {
  std::memcpy(data, static_cast<char*>(mmap_ptr_) + offset, n);
}

std::string MmapReader::label() const {
  return "mmap file " + label_;
}

void* MmapReader::data_at(size_t offset) const {
  return static_cast<char*>(mmap_ptr_) + offset;
}

allocator::Buffer MmapReader::get_metal_buffer() {
  if (!metal_buffer_.ptr() && mmap_ptr_) {
    metal_buffer_ = allocator::make_buffer(mmap_ptr_, file_size_);
  }
