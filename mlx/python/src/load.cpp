...
mlx_load_safetensor_helper(nb::object file, mx::StreamOrDevice s, bool use_mmap) {
  if (is_str_or_path(file)) { // Assume .safetensors file path string
    auto file_str = nb::cast<std::string>(nb::str(file));
    if (use_mmap) {
      return mx::load_safetensors_mmap(file_str, s);
    }
...
LoadOutputTypes mlx_load_helper(
  ...
  if (format.value() == "safetensors") {
    auto [dict, metadata] = mlx_load_safetensor_helper(file, s, use_mmap);
    if (return_metadata) {
